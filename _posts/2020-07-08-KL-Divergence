...
# Motivation
It was years ago when I first came across these concepts. I decided to record it since I have been frequently deriving the entropy relations.
...
## KLD Python Example 2: [How to Calculate the KL Divergence for Machine Learning](https://machinelearningmastery.com/divergence-between-probability-distributions/).
...
3. Next, we can develop a function to calculate the KL divergence between the two distributions. We will use log base-2 to ensure the result has units in bits.
```
...
def kl_divergence(p, q):
 ...
 4. We can then use this function to calculate the KL divergence of P from Q, as well as the reverse, Q from P: 
 ```
# calculate (P || Q)
...
It uses the KL divergence to calculate a normalized score that is symmetrical. This means that the divergence of P from Q is the same as Q from P: 
JS(P || Q) == JS(Q || P)
...
## JSD Python example
1. define a function to calculate the JS divergence that uses the kl_divergence() function prepared in the previous section: 
```
# calculate the kl divergence
def kl_divergence(p, q):
	return sum(p[i] * log2(p[i]/q[i]) for i in range(len(p)))
...
2. We can then test this function using the same probability distributions used in the previous section.
```
# calculate JS(P || Q)
...
```
show that the divergence is symmetrical: 
```
# calculate JS(Q || P)
...
* The SciPy library provides an implementation of the JS distance via the jensenshannon() function. It takes arrays of probabilities across all events from each probability distribution as arguments and returns the JS distance score, not a divergence score: 
```
...
1. we create a probability distribution with a known mean (0) and variance (2). Then, we create another distribution with random parameters:
```
x = np.arange(-10, 10, 0.001)
...
```
2. Given that we are using gradient descent, we need to select values for the hyperparameters (i.e. step size, number of iterations).
```
learning_rate = 0.001
epochs = 100
```
3. Just like numpy, in tensorflow we need to allocate memory for variables. For the variable q, we use the equation for a normal distribution given mu and sigma, only we exclude the part before the exponent since we’re normalizing the result.
...
```
4. Just like before, we define a function to compute the KL divergence that excludes probabilities equal to zero.
```
kl_divergence = tf.reduce_sum(
    tf.where(p == 0, tf.zeros(pdf.shape, tf.float64), p * tf.log(p / q))
)
```
5. Next, we initialize an instance of the GradientDescentOptimizer class and call the minimize method with the KL divergence function as an argument: 
```
optimizer = tf.train.GradientDescentOptimizer(learning_rate).minimize(kl_divergence)
```
6. Only after running tf.global_variables_initializer() will the variables hold the values we set when we declared them (i.e. tf.zeros).
```
init = tf.global_variables_initializer()
```
7. All operations in tensorflow must be done within a session. In the proceeding code block, we minimize the KL divergence using gradient descent. Then, we plot the probability distribution and KL divergence at different points in time. 
```
